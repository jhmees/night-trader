...
Only decisions that already carry a passing/clipped GuardVerdict may be
submitted — this module refuses anything else, so no code path can skip the
guard (RT-5 hard boundary: nothing executes from a chat interface).
...
class GuardBypassError(RuntimeError):
    """Raised when submission is attempted without a passing guard verdict."""
...
class PaperBroker:
    ...
    def submit(self, decision: Decision, verdict: GuardVerdict, equity_usd: float) -> OrderReceipt:
        _require_guard_approval(verdict)
...
def _require_guard_approval(verdict: GuardVerdict) -> None:
